<script lang="ts">
  ...
  function startEdit(user: UserSummaryDTO) {
    editedUser = user;
    selectedRoles = (user.roles ?? []).filter(
      (role): role is Role => role !== 'NONE'
    );
    editError = null;
  }
...
<div class="mx-auto w-full max-w-6xl px-4 py-8">
  <PageHeader
    title="Użytkownicy"
    description="Lista kont użytkowników w systemie."
  ...
  {:else if users.length === 0}
    ...
    />
  {:else}
    <UserTable users={users} onEditRoles={startEdit} />

    {#if page}
      <Pagination page={page} onPageChange={loadUsers} />
    {/if}
  {/if}
</div>