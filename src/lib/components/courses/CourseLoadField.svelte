<script lang="ts">
	...
	let { loads, selectedLoadIds, currentCourseId = null, onToggle }: Props = $props();
					...
					{#if load.id !== undefined && load.id !== null}
						...
						<div class="flex flex-col border border-zinc-200 bg-white p-2 text-sm text-black">
							...
							{#if isAssignedToAnotherCourse(load)}
								<p class="mt-1 ml-6 text-xs text-amber-700 select-none">
									Uwaga: ten ładunek jest już przypisany do kursu #{load.courseId}. Przypisanie go
									tutaj może przepiąć go do tego kursu.
								</p>
							{/if}
						</div>
					{/if}
